        ...
        if ($request->status == 'success'){
            ...
            if (isset($response['statusCode']) && $response['statusCode'] == "0000" && $response['transactionStatus'] == "Completed") {
             ...
             if ($getAvailableAmounts == null) {
                 ...
                 $getAvailableAmount->amount =  $total_amount; // Set default amount
                 $getAvailableAmount->available_amount = $availableAmount+$total_amount;
                 $getAvailableAmount->total_earn =$total_earn_amount+$total_amount;  // Add 100 to the existing total earnings
             ...
             //  $branch_id=$auth->branch_id;
               ...
               $getAvailableAmount->amount = $total_amount;  // Set default amount
                 $getAvailableAmount->available_amount = $availableAmount+$total_amount;
                 $getAvailableAmount->total_earn =$total_earn_amount+$total_amount;
               $getAvailableAmount->institute_id=$getAuth->branch_id;
               $getAvailableAmount->created_by= $auth;
               $getAvailableAmount->date =Carbon::createFromFormat('Y-m-d', '2024-10-02');
              $getAvailableAmount->save();
